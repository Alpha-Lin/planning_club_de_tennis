    ...
    if(!empty($_POST['date']) AND !empty($_POST['h_début']) AND !empty($_POST['h_fin']) AND !empty($_POST['entraîneur'])){
        if(isset( $_POST['user_adding_1']) AND !empty( $_POST['user_adding_1'])){
            ...
            if($req->fetch()){ // Vérifie si un cours est déjà programmé pour le jour, l'heure, et le prof sélectionnés
                ...
                do{
                    if(!in_array($_POST['user_adding_' . $idUser], $usersInserted)){ // On vérifie que l'élève n'a pas été sélectionné 2 fois
                        
                        $req->execute(array($_POST['user_adding_' . $idUser]));
                        $usersInserted[] = $_POST['user_adding_' . $idUser];
                    }
                    $idUser++;
                }while(isset($_POST['user_adding_' . $idUser]) AND !empty($_POST['user_adding_' . $idUser]));
